...
>>>>>>> Stashed changes
  ...
  def buscaLargura(self, verticeInicial):
    ''' BFS - Finds every node in the same level (the neighboors) and only after that, jumps to another layer '''
    inicio = time()
    if self.repr == "lista":
      visitas = {}
      queue = []
  ...
  def buscaDijkstra(self, verticeInicial, verticeFinal):
      ''' Finds the shortest path to reach a node based on the weights between edges'''
      if self.ponderado:
        inicio = time()
        predecessores = {}
        distanciaAcumulada = {}
 ...
 # ======================= Funções de representação ======================= #
  ...
  def warshall(self):
      ''' Transitive Closure of the graph, which returns the matrix representing the possible paths reached from each node '''
      # e.g, A -> B and B -> C, then A -> C 
      if self.repr == "matriz":
  ...
  def SCC(self): 
    ''' An strongly connected component is a directed graph where every node can reach any other node within the same set
    and every node can be reached by any other'''
    if self.direcionado:
      return len(self.componentsSCC()) == 1

  def euleriano(self): 
      ''' An Eulerian graph is a graph where you can reach every node by not repeating any edges and turning back to the inicial node '''
  ...
  def conexo(self): 
      ''' say a graph is connected means it has a path between every pair of nodes'''
      if not self.direcionado:
        vertice = random.choice(self.vertices)
  ...
  def prim(self):
    ''' Returns the minimum spanning tree of a graph, i.e, the tree generated by this greedy algorithm with the shortest paths to reach all nodes  '''
  ...
  def gerarHistograma(self, arq='histograma.png'):
    ''' Generates the histogram, so we can depict the degree distribution in the graph '''
